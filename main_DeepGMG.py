...
class Args_DGMG:
    def __init__(self):
        ...
        self.fname = (
            self.note + "_" + self.graph_type + "_" + str(self.node_embedding_size)
        )
        self.fname_pred = (
            self.note
            + "_"
...
def train_DGMG_epoch(epoch, args, model, dataset, optimizer, scheduler, is_fast=False):
    ...
    for i in order:
        ...
        node_count = 1
        node_embedding = [
            Variable(torch.ones(1, args.node_embedding_size)).cuda()
        ...
        while node_count <= graph.number_of_nodes():
            ...
            node_neighbor_new = graph.subgraph(
            ...
            if node_count < graph.number_of_nodes():
                ...
                if is_fast:
                    node_embedding_cat = torch.cat(node_embedding, dim=0)
                # calc loss
                loss_addnode_step = F.binary_cross_entropy(
                    p_addnode, Variable(torch.ones((1, 1))).cuda()
                )
                ...
                # calc loss
                loss_addnode_step = F.binary_cross_entropy(
                    p_addnode, Variable(torch.zeros((1, 1))).cuda()
                )
                # loss_addnode_step.backward(retain_graph=True)
                loss += loss_addnode_step
                loss_addnode += loss_addnode_step.data
            ...
            while edge_count <= len(node_neighbor_new):
                if not is_fast:
                    node_embedding = message_passing(
                        node_neighbor, node_embedding, model
                    )
                    node_embedding_cat = torch.cat(node_embedding, dim=0)
                    graph_embedding = calc_graph_embedding(node_embedding_cat, model)
                ...
                if edge_count < len(node_neighbor_new):
                    # calc loss
                    loss_addedge_step = F.binary_cross_entropy(
                        p_addedge, Variable(torch.ones((1, 1))).cuda()
                    )
                    ...
                    s_node = model.f_s(
                        torch.cat(
                            (node_embedding_cat[0:-1, :], node_new_embedding_cat), dim=1
                        )
                    )
                    ...
                    node_neighbor[node_neighbor_new[edge_count]].append(
                        len(node_neighbor) - 1
                    )
                    # calc loss
                    ...
                    # calc loss
                    loss_addedge_step = F.binary_cross_entropy(
                        p_addedge, Variable(torch.zeros((1, 1))).cuda()
                    )
                    # loss_addedge_step.backward(retain_graph=True)
                    loss += loss_addedge_step
                    loss_addedge += loss_addedge_step.data
...
def train_DGMG(args, dataset_train, model):
    ...
    if args.load:
        fname = (
            args.model_save_path + args.fname + "model_" + str(args.load_epoch) + ".dat"
        )
    ...
    while epoch <= args.epochs:
        ...
        if args.save:
            if epoch % args.epochs_save == 0:
                fname = (
                    args.model_save_path + args.fname + "model_" + str(epoch) + ".dat"
                )
...
def train_DGMG_nll(args, dataset_train, dataset_test, model, max_iter=1000):
    ...
    with open(fname_output, "w+") as f:
        ...
        for iter in range(max_iter):
            nll_train = train_DGMG_forward_epoch(
                args, model, dataset_train, is_fast=args.is_fast
            )
            nll_test = train_DGMG_forward_epoch(
                args, model, dataset_test, is_fast=args.is_fast
            )
