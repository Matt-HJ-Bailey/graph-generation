...
def evaluation(
    ...
    for model_name in model_name_all:
        for dataset_name in dataset_name_all:
            ...
            print(
                "processing: " + dir_output + model_name + "_" + dataset_name + ".csv"
            )
            logging.info(
                "processing: " + dir_output + model_name + "_" + dataset_name + ".csv"
            )
            if overwrite == False and os.path.isfile(fname_output):
                ...
                logging.info(
                    dir_output + model_name + "_" + dataset_name + ".csv exists!"
                )
                continue
            evaluation_epoch(
                dir_input,
...
def eval_list_fname(
    ...
    results = {
        ...
        "clustering": {"real": 0, "ours": 100, "perturbed": 0, "kron": 0},
        "orbits4": {"real": 0, "ours": 100, "perturbed": 0, "kron": 0},
    }
    ...
    for i in range(num_evals):
        ...
        print(
            "real average nodes",
            sum([real_g_list[i].number_of_nodes() for i in range(len(real_g_list))])
            / len(real_g_list),
        )
        print(
            "pred average nodes",
            sum([pred_g_list[i].number_of_nodes() for i in range(len(pred_g_list))])
            / len(pred_g_list),
        )
        ...
        results["clustering"]["ours"] = min(
            dist_clustering, results["clustering"]["ours"]
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--graph-type", dest="graph_type", help="Type of graphs / dataset."
    )
    ...
    if not os.path.isdir("logs/"):
        os.makedirs("logs/")
    logging.basicConfig(
        filename="logs/evaluate" + time_now + ".log", level=logging.INFO
    )
